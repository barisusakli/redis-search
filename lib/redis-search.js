...
Search.prototype.query = function(data, start, stop, callback) {
  ...
  for(var index in keyMap) {
    if (keyMap.hasOwnProperty(index)) {
      ...
      cmds.push(['zunionstore', tmpKeyName, keys.length].concat(keys));
    }
  }
  ...
  var finalKey = namespace + 'tmpFinal';
  if (tmpKeys.length === 1) {
    finalKey = tmpKeys[0];
  } else {
    cmds.push(['zinterstore', finalKey, tmpKeys.length].concat(tmpKeys));
  }
  ...
  cmds.push(['zremrangebyrank', finalKey, start, stop]);

  redisClient.multi(cmds).exec(function(err, ids) {
    if (err) {
      return callback(err);
    }
    callback(null, ids[tmpKeys.length === 1 ? 1 : tmpKeys.length + 1]);
  });
};
...
function getKeys(namespace, data) {
  ...
  for (var index in data) {
    if (data.hasOwnProperty(index)) {
      keys[index] = indexToKeys(namespace, index, data);
    }
  }
  return keys;
}

function indexToKeys(namespace, index, data) {
  var keys = [];
  if (index === 'content') {
    if (!data.content) {
      return keys;
    }
    var words = stripStopWords(toLowerCase(toWords(data.content)));
    words.forEach(function(word) {
      keys.push(namespace + ':' + index + ':' + word + ':id');
    });
  } else {
    if (Array.isArray(data[index])) {
      data[index].forEach(function(indexValue) {
        keys.push(namespace + ':' + index + ':' + indexValue + ':id');
      });
    } else {
      keys.push(namespace + ':' + index + ':' + data[index] + ':id');
    }
  }
...
function addIndexCommands(cmds, namespace, index, indexValue, id) {
  ...
  if (index === 'content') {
    var words = stripStopWords(toLowerCase(toWords(indexValue)));
    var counts = countWords(words);
    words.forEach(function(word) {
      cmds.push(['zadd', namespace + ':' + index + ':' + word + ':id', counts[word], id]);
      cmds.push(['sadd', namespace + ':id:' + id + ':' + index, word]);
    });
  } else {
